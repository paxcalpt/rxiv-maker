# API Documentation

Welcome to the API documentation for rxiv-maker.

## Commands Modules

- [commands.analyze_word_count.py](commands_analyze_word_count.py.md)
- [commands.copy_pdf.py](commands_copy_pdf.py.md)
- [commands.generate_docs.py](commands_generate_docs.py.md)
- [commands.generate_figures.py](commands_generate_figures.py.md)
- [commands.generate_preprint.py](commands_generate_preprint.py.md)

## Processors Modules

- [processors.author_processor.py](processors_author_processor.py.md)
- [processors.template_processor.py](processors_template_processor.py.md)
- [processors.yaml_processor.py](processors_yaml_processor.py.md)

## Converters Modules

- [converters.types.py](converters_types.py.md)

## Scripts Modules

- [scripts.custom_doc_generator.py](scripts_custom_doc_generator.py.md)
- [scripts.fix_minted.py](scripts_fix_minted.py.md)

## Debug Modules

- [debug.__init__.py](debug___init__.py.md)
- [debug.debug_conversion.py](debug_debug_conversion.py.md)
- [debug.debug_regex.py](debug_debug_regex.py.md)

## Core Modules

- [_version.py](_version.py.md)
- [template_processor.py](template_processor.py.md)
- [utils.py](utils.py.md)

