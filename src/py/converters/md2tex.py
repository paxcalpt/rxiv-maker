...
def convert_markdown_to_latex(content):
    ...
    content = re.sub(r"\b[\w]+_[\w._]*\.(md|yml|yaml|bib|tex|py|csv|pdf|png|svg|jpg)\b", escape_filenames, content)
...
def convert_code_blocks_to_latex(text):
    ...
    text = re.sub(r'^```(?:\w+)?\n(.*?)\n```$', process_fenced_code_block, text, flags=re.MULTILINE | re.DOTALL)
