"""Reference validator for checking cross-references to figures, tables, equations, and notes."""
...
class ReferenceValidator(BaseValidator):
    """Validates cross-references to figures, tables, equations, and supplementary notes."""
                ...
                if label_id in self.defined_labels[ref_type]:
                    ...
                    errors.append(
                        self._create_error(
                            ...
                            context=line,
                            suggestion=f"Label '{label_id}' is already defined in {existing['file']} at line {existing['line']}",
    ...
    def _suggest_correct_reference_type(self, label_id: str, current_type: str) -> str:
        ...
        for ref_type, labels in self.defined_labels.items():
            if ref_type != current_type and label_id in labels:
                ...
                return f"Label '{label_id}' exists as a {type_names.get(ref_type, ref_type)}. Use @{ref_type}:{label_id} instead."
