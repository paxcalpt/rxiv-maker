...
class FigureValidator(BaseValidator):
    ...
    def _validate_figure_attributes(self, figure_info: Dict, file_path: str, line_num: int) -> List:
        ...
        if 'tex_position' in attributes:
            ...
            valid_positions = {'h', 't', 'b', 'p', '!h', '!t', '!b', '!p', '!ht', '!hb', '!tb', '!htb'}
    ...
    def _check_unused_files(self) -> List:
        ...
        # Find unused files (excluding data files and hidden files)
        unused_files = []
        for file_path in self.available_files:
            if file_path not in referenced_files:
                # Skip files in DATA/ subdirectory and hidden files
                if not file_path.startswith('DATA/') and not os.path.basename(file_path).startswith('.'):
                    unused_files.append(file_path)
                    
        for unused_file in unused_files:
            warnings.append(self._create_error(
                ValidationLevel.INFO,
