...
class TestDockerMultiArch:
    ...
    def test_dockerfile_has_multiarch_support(self):
        ...
        ), "Should have buildx syntax directive"
        assert (
    ...
    def test_platform_detection(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=0, stdout="Platforms: linux/amd64, linux/arm64, linux/arm/v7"
        )
    ...
    def test_platform_specific_optimizations(self):
        ...
        if dockerfile_path.exists():
            ...
            assert (
                "apt-get clean" in content
            ), "Should include cleanup for smaller layers"
    ...
    def test_registry_authentication(self, mock_run):
        ...
        result = subprocess.run(
            ["docker", "login", "-u", "testuser", "--password-stdin"],
            input="testpass",
            text=True,
            capture_output=True,
        )
    ...
    def test_cache_optimization_config(self):
        ...
        if dockerfile_path.exists():
            ...
            copy_lines = [
                ...
                if "COPY" in line and "pyproject.toml" in line
            ]
            pip_lines = [
                i
                for i, line in enumerate(lines)
                if "pip install" in line or "uv pip install" in line
            ]
...
class TestBuildScript:
    ...
    def test_build_script_error_handling(self):
        ...
        if script_path.exists():
            ...
            assert (
                "log_error" in content or "echo" in content
