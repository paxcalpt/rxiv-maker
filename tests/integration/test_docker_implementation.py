...
class TestDockerImageFunctionality:
    ...
    def test_docker_node_and_mermaid_functionality(self):
        ...
        try:
            ...
            assert (
                "v20" in result.stdout or "v18" in result.stdout
            ), "Should have Node.js v18 or v20"
...
"""
            ...
            (manuscript_dir / "03_REFERENCES.bib").write_text(
                "% Empty bibliography for testing"
            )
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    assert (
                        "Docker Test Manuscript" in tex_content
                    ), "Should contain manuscript title"
                    ...
                    if "pdflatex" in result.stderr or "LaTeX" in result.stderr:
                        pytest.skip(
                            f"LaTeX processing issue in Docker: {result.stderr}"
                        )
                    else:
                        pytest.fail(
                            f"Docker manuscript generation failed: {result.stderr}"
                        )
    ...
    def test_docker_platform_architecture_detection(self):
        ...
        try:
            ...
            assert (
                result.returncode == 0
            ), f"Architecture detection failed: {result.stderr}"
...
class TestDockerEnvironmentVariables:
    ...
    def test_environment_variable_passing(self):
        ...
        try:
            ...
            assert (
                result.returncode == 0
            ), f"Environment variable test failed: {result.stderr}"
            assert (
                "docker_test_value" in result.stdout
            ), "Environment variable should be passed"
    ...
    def test_manuscript_path_environment_variable(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                assert (
                    result.returncode == 0
                ), f"Custom manuscript path test failed: {result.stderr}"
